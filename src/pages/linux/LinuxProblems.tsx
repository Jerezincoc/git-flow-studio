...
import { linuxProblems, linuxProblemCategoryLabels } from "@/data/linuxProblems";

const filterOptions = [
  { value: "all", label: "Todos" },
  ...Object.entries(linuxProblemCategoryLabels).map(([value, label]) => ({ value, label })),
];
...
export default function LinuxProblems() {
  ...
  const filtered =
    filter === "all" ? linuxProblems : linuxProblems.filter((p) => p.category === filter);

  return (
    <div data-theme="linux" className="container px-4 py-12 max-w-4xl">
      <SEO
        title="Problemas Linux"
        description="Base de problemas comuns no Linux com causa, solução passo a passo e dicas de prevenção."
        path="/linux/problems"
      />
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-3xl font-bold mb-2">Problemas Comuns</h1>
        <p className="text-muted-foreground mb-6">
          {linuxProblems.length} problemas documentados com causa, solução passo a passo e prevenção.
        </p>
      </motion.div>

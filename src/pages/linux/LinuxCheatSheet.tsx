...
import { linuxCommands, linuxCategoryLabels } from "@/data/linuxCommands";
...
export default function LinuxCheatSheet() {
  ...
  const filtered = filter === "all" ? linuxCommands : linuxCommands.filter((c) => c.category === filter);

  const grouped = Object.entries(linuxCategoryLabels)
    .map(([cat, label]) => ({
      category: cat,
      label,
  ...
  return (
    <div data-theme="linux" className="container px-4 py-12 max-w-5xl">
      <SEO
        title="Linux Cheat Sheet"
        description="Referência rápida de comandos Linux essenciais organizados por categoria, com sintaxe e cópia rápida."
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-start justify-between mb-2 flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Referência rápida de comandos Linux. Filtre por categoria e copie a sintaxe.
            </p>
          </div>
          <button
                ...
                {group.items.map((cmd) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <code className="text-xs text-muted-foreground/60 font-mono">{cmd.syntax}</code>
                    </div>
        ...
        <p className="text-xs text-muted-foreground mt-12 text-center print:hidden">
          ...
          <Link to="/linux/commands" className="hover:text-primary">

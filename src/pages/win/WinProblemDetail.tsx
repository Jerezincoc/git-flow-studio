...
import { AlertCircle, ArrowLeft, Lightbulb, ShieldCheck } from "lucide-react";
import { CopyButton } from "@/components/CopyButton";
import { Button } from "@/components/ui/button";
import { getWinProblemById, winProblemCategoryLabels } from "@/data/winProblems";
...
export default function WinProblemDetail() {
  const { id } = useParams<{ id: string }>();
  const problem = getWinProblemById(id ?? "");

  if (!problem) {
    return (
      <div data-theme="win" className="container px-4 py-20 text-center">
        <p className="text-muted-foreground">Problema nao encontrado.</p>
        <Button asChild variant="outline" className="mt-4"><Link to="/win/problems">Voltar</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div data-theme="win" className="container px-4 py-10 max-w-3xl">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <Link to="/win/problems" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6">
          <ArrowLeft className="h-4 w-4" /> Voltar aos problemas
        </Link>

        <div className="flex items-start gap-3 flex-wrap mb-2">
          <h1 className="text-2xl font-bold flex-1">{problem.title}</h1>
          <span className={`text-xs px-2 py-1 rounded-full font-medium shrink-0 ${difficultyColor[problem.difficulty]}`}>{difficultyLabel[problem.difficulty]}</span>
        </div>
        ...
        <div className="mt-8">
          <h2 className="text-base font-semibold mb-4">Solucao passo a passo</h2>
          <div className="space-y-4">
            {problem.steps.map((step, index) => (
              <div key={index} className="glass-card p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="w-5 h-5 rounded-full bg-primary/20 text-primary text-xs font-bold flex items-center justify-center shrink-0">{index + 1}</span>
        ...
        {problem.prevention && (
          <div className="mt-6 glass-card p-4 flex gap-3 border-primary/20">
            ...
            <div>
              <p className="text-xs font-semibold text-primary mb-1">Prevencao</p>
        ...
        {problem.relatedCommands.length > 0 && (
          <div className="mt-8">
            ...
            <div className="flex flex-wrap gap-2">
              {problem.relatedCommands.map((command) => (
                <Link key={command} to={`/win/commands/${command}`} className="font-mono text-sm bg-secondary hover:bg-secondary/80 px-3 py-1.5 rounded-lg text-primary transition-colors">{command}</Link>
              ))}
            </div>
          </div>
        )}

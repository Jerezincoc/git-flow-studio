...
export default function ShellCommandDetail() {
  const { id } = useParams<{ id: string }>();
  const cmd = getShellCommandById(id ?? "");
  const [favorites, setFavorites] = useLocalStorage<string[]>("shell-doc-favorites", []);
  const [history, setHistory]     = useLocalStorage<string[]>("shell-doc-history", []);
  ...
  if (!cmd) {
    return (
      <div className="container px-4 py-20 text-center">
        <p className="text-muted-foreground">Cmdlet não encontrado.</p>
        <Button asChild variant="outline" className="mt-4">
          <Link to="/shell/commands">Voltar</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div data-theme="shell" className="container px-4 py-10 max-w-3xl">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <Link
          to="/shell/commands"
          className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-6"
        >
          <ArrowLeft className="h-4 w-4" /> Voltar aos cmdlets
        </Link>
        ...
        <div className="mt-8">
          <Tabs defaultValue="geral">
            ...
            <TabsContent value="geral" className="mt-6 space-y-8">
              ...
              <Section title="Cmdlets relacionados">
                  ...
                  {cmd.flags.map((f, i) => (
                    <div key={i} className={`glass-card p-3 ${f.danger ? "border border-destructive/30" : ""}`}>
                      <div className="flex items-start gap-2">
                        ...
                        {f.danger && (
                          <AlertTriangle className="h-3.5 w-3.5 text-destructive mt-0.5 shrink-0" title="Use com cuidado" />
                        )}

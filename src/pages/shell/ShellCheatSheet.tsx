...
export default function ShellCheatSheet() {
  ...
  const filtered = filter === "all" ? shellCommands : shellCommands.filter((c) => c.category === filter);

  const grouped = Object.entries(shellCategoryLabels)
    .map(([cat, label]) => ({
      category: cat,
      label,
  ...
  return (
    <div data-theme="shell" className="container px-4 py-12 max-w-5xl">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-start justify-between mb-2 flex-wrap gap-3">
          <div>
            <h1 className="text-3xl font-bold">Cheat Sheet</h1>
            <p className="text-muted-foreground mt-1">
              Referência rápida de todos os cmdlets. Clique em qualquer um para ver detalhes.
            </p>
          </div>
          <button
        ...
        <p className="text-xs text-muted-foreground mt-12 text-center print:hidden">
          {filtered.length} cmdlets ·{" "}
          <Link to="/shell/commands" className="hover:text-primary">

...
import { getShellProblemById, shellProblemCategoryLabels } from "@/data/shellProblems";
...
export default function ShellProblemDetail() {
  const { id } = useParams<{ id: string }>();
  const problem = getShellProblemById(id ?? "");
    ...
    return (
      <div className="container px-4 py-20 text-center">
        ...
        <Button asChild variant="outline" className="mt-4">
          <Link to="/shell/problems">Voltar</Link>
        </Button>
      </div>
    );
  }
    ...
    <div data-theme="shell" className="container px-4 py-10 max-w-3xl">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        {problem.relatedCommands.length > 0 && (
          <div className="mt-8">
            <h2 className="text-base font-semibold mb-3">Cmdlets relacionados</h2>
            <div className="flex flex-wrap gap-2">
              {problem.relatedCommands.map((cmd) => (
                <Link
                  key={cmd}
                  to={`/shell/commands/${cmd}`}
